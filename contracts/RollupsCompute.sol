...
contract RollupsCompute {
    ...
    /// @param _consensus consensus to use for computation instances
    constructor(ICartesiDAppFactory _factory, IInputBox _inputBox, IConsensus _consensus) {
        factory = _factory;
        inputBox = _inputBox;
        ...
        returns (CartesiDApp)
    {
        CartesiDApp dapp = factory.newApplication(
            ...
            address(this),
            _templateHash,
            calculateSalt(_templateHash, _id)
        );
        ...
        // - possible solution: instantiate() could also add an input to the instance with the mainDApp address (like a "setup input")
        InstanceInfo storage instance = instances[address(dapp)];
    ...
    function relay(bytes _data) external {
        InstanceInfo instance = instances[msg.sender];
